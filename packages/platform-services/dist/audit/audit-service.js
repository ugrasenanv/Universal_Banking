...
const shared_types_1 = require("@afg/shared-types");
/**
...
class AuditService {
    ...
    async persist(input) {
        ...
        const retentionExpiryDate = (0, shared_types_1.computeRetentionExpiry)(input.timestamp);
        ...
        const integrityHash = (0, shared_types_1.computeIntegrityHash)(contentForHash);
     ...
     */
    async getById(artefactId, jurisdiction) {
     ...
     */
    async verifyIntegrity(artefactId, jurisdiction) {
        const artefact = await this.store.getById(artefactId, jurisdiction);
        if (!artefact) {
            throw new Error(`Artefact not found: ${artefactId} in jurisdiction ${jurisdiction}`);
        }
        const { integrityHash, ...content } = artefact;
        const computedHash = (0, shared_types_1.computeIntegrityHash)(content);
        return {
            artefactId,
            valid: (0, shared_types_1.verifyIntegrity)(artefact),
     ...
     */
    isRetentionExpired(artefact, asOf) {
