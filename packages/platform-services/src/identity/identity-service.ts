...
export class IdentityService {
  ...
  selectSCAMethod(availableMethods: SCAMethod[], preferredMethod?: SCAMethod): SCAMethod {
    ...
    const priority: SCAMethod[] = ['BIOMETRIC', 'HARDWARE_TOKEN', 'OTP'];
  ...
  private validateCredentials(request: AuthenticationRequest): boolean {
  ...
  private performSCA(method: SCAMethod): boolean {
    ...
    const supportedMethods: SCAMethod[] = ['BIOMETRIC', 'OTP', 'HARDWARE_TOKEN'];
    return supportedMethods.includes(method);
  }

  private createSession(request: AuthenticationRequest, scaCompleted: boolean): Session {
    const now = new Date();
    const maxSessionMs = this.sessionConfig.maxSessionHours * 60 * 60 * 1000;
  ...
  private checkSegregationOfDuty(assignedRoles: string[]): SegregationRule | null {
  ...
  private checkRBACPermission(request: AccessRequest): boolean {
    const { assignedRoles, resource, action } = request;

    // Find all roles that match the assigned role IDs
    const matchedRoles = this.accessPolicy.rbac.filter((role) =>
      assignedRoles.includes(role.roleId),
    );

    // Check if any matched role grants the requested permission
    return matchedRoles.some((role) =>
  ...
  private checkABACConditions(request: AccessRequest): { passed: boolean; failedCondition?: string } {
  ...
  private evaluateCondition(condition: AttributeCondition, actualValue: string | number | string[]): boolean {
